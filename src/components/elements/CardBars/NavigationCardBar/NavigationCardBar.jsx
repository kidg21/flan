...
function NavigationCardBar({ id, title, number, disabled, toggle, onStateChange, onClick, switchProps }) {
	...
	const titleComponent = (<Title title={`${state.disabled ? "DISABLED - " : ""}${title}`} weight="normal" />);
	let leftComponent = null;
	if (state.toggle === true) {
		leftComponent = (<SwitchContainer>
			{" "}
			<Switch checked={!state.disabled} onStateChange={(oldSwitchState, newSwitchState, setSwitchState, e) => {
				const oldState = Object.assign({}, state);
				const newState = Object.assign({}, state, { disabled: !newSwitchState.checked });
				setState(newState);
				if(e){
					e.stopPropagation();
				}
