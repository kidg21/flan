...
anchor element is `<Button icon="options" isPlain isRound />`. If you want to change the anchor's
style (i.e. Button is not round, or icon should be menu) or use a different component (`<Icon/>`),
you can specify it as a child of `Menu`.
...
If a `visible` prop isn't set, `Menu` will handle its own state to open and close. If you want a parent component to control
opening/closing, pass in `visible` to `Menu`. Use `Menu`'s' `onClose` prop and `onClick` prop of an anchor element to help toggle
the visibility. With a stateless `Menu` it gives you the flexibility to control its visibilty from outside components.
...
For cases when `Menu` needs to break out of the DOM hierarchy, use the `usePortal` prop. It places the
...
- **Default anchor element switched from `<Icon icon={icon} />` to `<Button icon="options" isPlain isRound />`** -
Making the default a `<Button />` makes the anchor element easier to target. If you want `<Icon />` to be
your anchor element you will need to pass it in as a child.

- **Removed `icon` prop** -
For an anchor element with a different icon than `options`, you will need to remove this
prop and pass a custom anchor element with your desired icon.

- **Removed `onClick` prop** -
For a custom `onClick` callback when clicking on the anchor element, you will need to remove this
prop and attach the `onClick` event to the custom anchor element.

- **Removed `isButton` prop** -
We updated the default to be a `<Button />`. You will just need to remove this prop from your code.
If the default anchor element isn't quite right, pass in a custom anchor element.
